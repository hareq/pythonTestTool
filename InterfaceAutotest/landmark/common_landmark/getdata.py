...
def metrostation():
    ...
    conn=pymssql.connect(host="hoteldb.uat.qa.nt.ctripcorp.com:55888",database="ArchSearchDB")
    cur=conn.cursor()
    cur.execute("select top 100 glat as lat,glon as lon,landmarkname as name,landmarkid as id,'metrostation' as type,'metrostation_' + cast(landmarkid as varchar) as identityid,c.*,newid() random from type_metrostation(nolock) as metro left join city(nolock) as c on metro.city=c.cityid where countryid = '1' order by random")
    row = cur.fetchone()
    while row:
        all.append(row)
        row = cur.fetchone()
...
def airport():
    ...
    conn=pymssql.connect(host="hoteldb.uat.qa.nt.ctripcorp.com:55888",database="ArchSearchDB")
    cur=conn.cursor()
    cur.execute("SELECT top 100 lat,lon,name,marklandid as id,'airport' as type,'airport_' + cast(marklandid as varchar) as identityid,c.*,newid() random from hotel_markland(nolock) as hm left join city(nolock) as c on hm.cityid=c.cityid where DicationaryType = 'jc2' and countryid = '1' order by random")
    row = cur.fetchone()
    while row:
        all.append(row)
        row = cur.fetchone()
...
def railwaystation():
    ...
    conn=pymssql.connect(host="hoteldb.uat.qa.nt.ctripcorp.com:55888",database="ArchSearchDB")
    cur=conn.cursor()
    cur.execute("SELECT top 100 lat,lon,name,marklandid as id,'railwaystation' as type,'railwaystation_' + cast(marklandid as varchar) as identityid,c.*,newid() random from hotel_markland(nolock) as hm left join city(nolock) as c on hm.cityid=c.cityid where hm.DicationaryType = 'hcz' and countryid = '1' order by random")
    row = cur.fetchone()
    while row:
        all.append(row)
        row = cur.fetchone()
